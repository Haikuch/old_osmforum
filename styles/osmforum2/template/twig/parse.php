...
class Parser
{
	public function parse($code, $filename = null)
	{
		// Our phpBB tags
...
function parseFiles($dir){
    ...
    foreach ( $ffs as $ff ){
        ...
        if ( strlen($ff)<5 OR substr($ff, -5) != '.html' ) {

            continue;
        }
             
        $code = file_get_contents($dir.'/'.$ff);
        
        $newcode = $parser->parse($code);
        
        file_put_contents($dir.'/'.$ff, $newcode);
        
        echo $ff . ' -> ' . substr($ff, 0, strlen($ff)-5).'.twig.html<br>';
